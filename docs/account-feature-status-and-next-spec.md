...
## 11. 2026-08-28 보완 작업 반영

### 11.1 마이페이지 계정 작업

- `/mypage` 계정 개요에서 이름 수정, 판매자 등록 신청, 회원탈퇴 UI를 제공한다.
- 이름 수정은 `PATCH /api/v1/account/me`를 호출하고 성공 응답으로 계정 요약을 즉시 갱신한다.
- 판매자 신청은 일반 회원이 아직 신청하지 않은 경우에만 입력 폼을 표시한다.
- 판매자 상태가 `PENDING`, `APPROVED`, `REJECTED`이면 중복 신청 폼 대신 상태별 안내를 표시한다.
- 회원탈퇴는 현재 비밀번호, 명시적 확인 체크, 최종 확인 대화상자를 모두 거친다.
- 탈퇴 성공 시 브라우저 Access Token을 삭제하고 `/login?withdrawn=success`로 이동한다.

### 11.2 인증 화면 보완

- 로그인·회원가입 화면은 브라우저 유효성 검사와 서버 `fieldErrors`를 입력 필드에 연결하고 `aria-invalid`를 설정한다.
- 로그인 오류는 잘못된 자격 증명, 정지 계정, 탈퇴 계정을 구분해 표시한다.
- 로그인 화면에서 저장된 토큰으로 `/api/v1/account/me` 조회가 성공하면 안전한 내부 경로로 자동 이동한다.
- 로그인 복귀 경로는 `/`로 시작하는 동일 origin 내부 경로만 허용하며 `//`와 역슬래시가 포함된 경로는 거부한다.
- 회원가입 성공과 회원탈퇴 성공은 로그인 화면의 query 상태 메시지로 전달한다.

### 11.3 거래 조회 어댑터 경계

- 마이페이지의 구매 요청, 구매, 판매, 받은 오퍼, 보낸 오퍼, 채팅 조회는 `mypage-api-adapters.js`에서 endpoint와 DTO 변환을 분리한다.
- 화면 렌더러는 도메인 DTO를 직접 참조하지 않고 `key`, `title`, `detail`, `status`, `meta` 표준 모델만 사용한다.
- 목록 응답은 배열과 `content`, `items`, `data` 래퍼를 수용하며 페이지 메타데이터 연결 공간을 유지한다.
- 도메인 작업물 병합 전에는 API 네임스페이스를 변경하지 않는다.

### 11.4 후속 작업 경계

- 실제 거래 DTO 필드 확정, 페이지네이션 버튼, 상세 화면 링크는 각 도메인 API 병합 후 어댑터에서 연결한다.
- WebSocket/STOMP의 JWT 인증, 채팅방 참여자 구독·발행 권한 검증은 채팅 도메인 병합 후 공동 리팩터링한다.
- Refresh Token과 서버 로그아웃 API는 기존 합의대로 현재 범위에 포함하지 않는다.
