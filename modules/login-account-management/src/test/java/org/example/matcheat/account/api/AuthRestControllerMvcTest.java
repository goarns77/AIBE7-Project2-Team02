...
import org.example.matcheat.account.api.auth.AuthRestController;
...
class AuthRestControllerMvcTest {
    ...
    @Mock
    private CheckEmailAvailabilityUseCase emailAvailabilityUseCase;
    ...
    void setUp() {
        AuthRestController controller = new AuthRestController(
                signUpUseCase, loginUseCase, emailAvailabilityUseCase);
    ...
    void returnsCreatedSignupResponseWithoutPasswordFields() throws Exception {
        when(signUpUseCase.signUp(any())).thenReturn(new SignUpUseCase.SignUpResult(
    ...
    void returnsLoginContract() throws Exception {
        when(loginUseCase.login(any())).thenReturn(new LoginUseCase.LoginResult(
                "token", "Bearer", 3600,
                new LoginUseCase.UserSummary(1L, "user@example.com", "홍길동", UserRole.USER)));
    ...
    void returnsEmailAvailabilityContract() throws Exception {
        when(emailAvailabilityUseCase.check("user@example.com"))
                .thenReturn(new CheckEmailAvailabilityUseCase.EmailAvailability("user@example.com", true));
