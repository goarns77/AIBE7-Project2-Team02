...
public class QuoteNegotiationController {
	...
	@GetMapping
	public ResponseEntity<QuoteNegotiationResponse> get(@PathVariable Long chatRoomId) {
		return ResponseEntity.ok(quoteNegotiationService.getNegotiation(chatRoomId, resolveCurrentUserId()));
	}
	...
	public ResponseEntity<QuoteNegotiationResponse> triggerAiSummary(@PathVariable Long chatRoomId) {
		return ResponseEntity.ok(quoteNegotiationService.triggerAiSummary(chatRoomId, resolveCurrentUserId()));
	}
	...
	@PostMapping("/lock")
	public ResponseEntity<QuoteNegotiationResponse> lock(@PathVariable Long chatRoomId) {
		return ResponseEntity.ok(quoteNegotiationService.lockNegotiation(chatRoomId, resolveCurrentUserId()));
	}

	// TODO: SecurityContext 적용 시 교체 (기존 QuoteController와 동일한 패턴)
